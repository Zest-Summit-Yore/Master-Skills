...
### Mandatory template, numbering, and final-layout discipline

请严格调用 documents skill 制作 Word。沿用所附模板的页面设置、字体、段落样式和多级编号；编号必须使用 Word 原生自动编号，不要手工输入编号。完成后逐页渲染检查，重点核对编号连续性、缩进、分页、表格跨页和字体替换，发现问题后修改并重新渲染，确认无误后再交付。

Treat this as an operational requirement for every applicable Word task. When a template is supplied, reuse its page setup, fonts, paragraph styles, and multilevel-list definitions instead of rebuilding or approximating them. Implement numbering with native Word numbering definitions and paragraph-style bindings; never type list markers or numbers into paragraph text. Before delivery, render and inspect every page, explicitly checking numbering continuity, hierarchy and indentation, page breaks, tables spanning pages, and font substitution. Correct every defect and repeat the render-and-inspect cycle until the latest render passes.
